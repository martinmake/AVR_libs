...
#define DEFINITION_CTOR_COPY(class_name, ...) inline class_name::class_name(const class_name&  other) : class_name(__VA_ARGS__) { copy(          other ); }
#define DEFINITION_CTOR_MOVE(class_name, ...) inline class_name::class_name(      class_name&& other) : class_name(__VA_ARGS__) { move(std::move(other)); }
...
#define DEFINITION_MANDATORY(class_name, ...)           \
	DEFINITION_CTOR_COPY(class_name, __VA_ARGS__)   \
	DEFINITION_CTOR_MOVE(class_name, __VA_ARGS__)   \
	                                                \
	DEFINITION_ASSIGNMENT_OPERATOR_COPY(class_name) \
	DEFINITION_ASSIGNMENT_OPERATOR_MOVE(class_name)

#define MANDATORY(class_name)                                    \
	public:                                                  \
...
#define MANDATORY_INTERFACE(class_name)                          \
	public:                                                  \
		DECLARATION_CTOR_COPY(class_name)                \
		DECLARATION_CTOR_MOVE(class_name)                \
                                                                 \
